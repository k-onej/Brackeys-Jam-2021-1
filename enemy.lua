...
function Enemy.new(x,y) --creats new Enemy class
  ...
  instance.physics.shape = love.physics.newRectangleShape(instance.width * 0.4, instance.height * 0.7)
...
function Enemy:animate(dt) --animates the player by changing the frames at 10fps
...
function Enemy:setNewFrame() --sets the new frame by adding to anim.current until it reaches anim.total
...
function Enemy:syncPhysics() --does same as what it does in player.lua ig lmao
